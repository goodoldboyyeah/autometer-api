...
public class ApicasesController {
    ...
    public Result add(@RequestBody Apicases apicases) {
        apicases.setCasename(apicases.getCasename().replaceAll("\\s+", ""));
        ...
        if (apicasesService.ifexist(con) > 0) {
            ...
            Deployunit deployunit = deployunitService.getById(apicases.getDeployunitid());
            apicases.setProtocal(deployunit.getProtocal());
            apicases.setServicetype(deployunit.getServicetype());
            apicasesService.save(apicases);
            //增加初始化参数值

            if (apicases.getServicetype().equals("Http服务")) {
            ...
            if (apicases.getServicetype().equals("Dubbo服务")) {
                Long apiid = apicases.getApiid();
                Condition con1 = new Condition(DubboapiParams.class);
                con1.createCriteria().andCondition("apiid = " + apiid);
                List<DubboapiParams> dubboapiParamsList = dubboapiParamsService.listByCondition(con1);
                for (DubboapiParams dubboapiParams : dubboapiParamsList) {
                    DubboapiCasedata dubboapiCasedata1 = new DubboapiCasedata();
                    dubboapiCasedata1.setId(null);
                    dubboapiCasedata1.setCaseid(apicases.getId());
                    dubboapiCasedata1.setApiparam(dubboapiParams.getKeyname());
                    dubboapiCasedata1.setApiparamvalue(dubboapiParams.getKeydefaultvalue());
                    dubboapiCasedata1.setParamstype(dubboapiParams.getParamstype());
                    dubboapiCasedata1.setMid(apicases.getMid());
                    dubboapiCasedata1.setCasename(apicases.getCasename());
                    dubboapiCasedata1.setMemo(apicases.getMemo());
                    dubboapiCasedataService.save(dubboapiCasedata1);
                }
                Dubboapi dubboapi = dubboapiService.getById(apiid);
                long casecount = dubboapi.getCasecounts();
                dubboapi.setCasecounts(casecount + 1);
                dubboapiService.update(dubboapi);
            }
            if (apicases.getServicetype().equals("WebSocket服务")) {
                Long apiid = apicases.getApiid();
                Condition con1 = new Condition(WsapiParams.class);
                con1.createCriteria().andCondition("apiid = " + apiid);
                List<WsapiParams> WSParamsList = wsapiParamsService.listByCondition(con1);
                for (WsapiParams wsapiParams : WSParamsList) {
                    WsapiCasedata wsapiCasedata1 = new WsapiCasedata();
                    wsapiCasedata1.setId(null);
                    wsapiCasedata1.setCaseid(apicases.getId());
                    wsapiCasedata1.setApiparamvalue(wsapiParams.getKeydefaultvalue());
                    wsapiCasedata1.setMid(apicases.getMid());
                    wsapiCasedata1.setCasename(apicases.getCasename());
                    wsapiCasedata1.setMemo(apicases.getMemo());
                    wsapiCasedataService.save(wsapiCasedata1);
                }
                Wsapi wsapi = wsapiService.getById(apiid);
                long casecount = wsapi.getCasecounts();
                wsapi.setCasecounts(casecount + 1);
            ...
            if (apiService.ifexist(apicon) == 0) {
                ...
                for (Api SourceApi : SourceapiList) {
                    ...
                    for (Apicases SourceCase : apicasesList) {
                        ...
                        DesitionApicase.setProtocal(SourceCase.getProtocal());
                        DesitionApicase.setServicetype(SourceCase.getServicetype());
...
//        }
        ...
        Condition testvariablesCondition = new Condition(Testvariables.class);
        testvariablesCondition.createCriteria().andCondition("caseid = " + id);
        testvariablesService.deleteByCondition(testvariablesCondition);

        if(apicases.getServicetype().equals("WebSocket服务"))
        {
            Wsapi api = wsapiService.getById(apicases.getApiid());
            long casecount = api.getCasecounts();
            api.setCasecounts(casecount - 1);
            wsapiService.updateDic(api);
            wsapiCasedataService.deletcasedatabyid(id);
        }
        if(apicases.getServicetype().equals("Dubbo服务"))
        {
            Dubboapi api = dubboapiService.getById(apicases.getApiid());
            long casecount = api.getCasecounts();
            api.setCasecounts(casecount - 1);
            dubboapiService.updateDic(api);
            dubboapiCasedataService.deletcasedatabyid(id);
        }
        if(apicases.getServicetype().equals("Http服务"))
        {
    ...
    public Result runscenetest(@RequestBody final Map<String, Object> param) throws Exception {
        ...
        for (TestsceneTestcase te : testsceneTestcaseList) {
            ...
            if (apicases.getServicetype().equals("Dubbo服务")) {
                String DeployunitName = apicases.getDeployunitname();
                String ModelName = apicases.getModelname();
                long Dubboapiid = apicases.getApiid();
                List<Macdepunit> macdepunitList = macdepunitService.getenvassemblelistbyenvidandtype(Long.parseLong(enviromentid), "组件");
                boolean existzk = false;
                String IP = "";
                String Port = "";
                String ConnectStr = "";
                for (Macdepunit macdepunit : macdepunitList) {
                    long assembleid = macdepunit.getAssembleid();
                    EnviromentAssemble enviromentAssemble = enviromentAssembleService.getById(assembleid);
                    if (enviromentAssemble.getAssembletype().equals("zookeeper") || enviromentAssemble.getAssembletype().equals("nacos")) {
                        Long MachineID = macdepunit.getMachineid();
                        Machine machine = machineService.getBy("id", MachineID);
                        IP = machine.getIp();
                        Port = enviromentAssemble.getConnectstr();
                        ConnectStr = enviromentAssemble.getConnectstr();
                        existzk = true;
                        break;
                    }
                }
                if (!existzk) {
                    return ResultGenerator.genFailedResult("当前选择环境未配置注册中心Zookeeper");
                } else {
                    Dubboapi dubboapi = dubboapiService.getById(Dubboapiid);
                    String Version = dubboapi.getVersion();
                    String Group = dubboapi.getDgroup();
                    // 获取参数
                    DubboTestResponeData dubboTestResponeData = new DubboTestResponeData();
                    dubboTestResponeData.setDeployunitName(DeployunitName);
                    dubboTestResponeData.setInterfaceService(ModelName);
                    dubboTestResponeData.setVersion(Version);
                    dubboTestResponeData.setGeneral("微服务：" + DeployunitName + "\n" + "Interface: " + ModelName + "\n" + "Version: " + Version + "\n" + "接口: " + dubboapi.getApiname());
                    Condition dubbocasedatacon = new Condition(DubboapiCasedata.class);
                    dubbocasedatacon.createCriteria().andCondition("caseid = " + apicases.getId());
                    List<DubboapiCasedata> dubboapiCasedataList = dubboapiCasedataService.listByCondition(dubbocasedatacon);
                    List<String> ParamsTypeList = new ArrayList<>();
                    List<String> ParamsValueList = new ArrayList<>();
                    String RequestData = "";
                    for (DubboapiCasedata dubboapiCasedata : dubboapiCasedataList) {
                        String ParamName = dubboapiCasedata.getApiparam();
                        String ParamValue = dubboapiCasedata.getApiparamvalue();
                        //1.替换随机变量
                        for (Variables variables : variablesList) {
                            String VariableName = "[" + variables.getVariablesname() + "]";
                            if (ParamValue.contains(VariableName)) {
                                Object VariableValue = GetRadomValue(variables.getVariablesname());
                                ParamValue = ParamValue.replace(VariableName, VariableValue.toString());
                            }
                        }
                        //2.替换接口变量
                        for (String Interfacevariables : ParamsValuesMap.keySet()) {
                            String UseInterfacevariables = "<" + Interfacevariables + ">";
                            if (ParamValue.contains(UseInterfacevariables)) {
                                String VariableValue = ParamsValuesMap.get(Interfacevariables);// GetVariablesObjectValues("$" +Interfacevariables, ParamsValuesMap);
                                int index = VariableValue.indexOf(",");
                                ParamValue = ParamValue.replace(UseInterfacevariables, VariableValue.substring(index + 1));
                            }
                        }
                        //3.替换数据库变量
                        for (String DBvariables : DBParamsValuesMap.keySet()) {
                            String UseDBvariables = "<<" + DBvariables + ">>";
                            if (ParamValue.contains(UseDBvariables)) {
                                String VariableValue = DBParamsValuesMap.get(DBvariables);
                                int index = VariableValue.indexOf(",");
                                ParamValue = ParamValue.replace(UseDBvariables, VariableValue.substring(index + 1));
                            }
                        }
                        //4.替换全局变量
                        for (Globalvariables variables : globalvariablesList) {
                        ...
                        for (String Scriptvariables : ScriptParamsValuesMap.keySet()) {
                            String UseScriptvariables = "{" + Scriptvariables + "}";
                            if (ParamValue.contains(UseScriptvariables)) {
                                String VariableValue = ScriptParamsValuesMap.get(Scriptvariables);
                                int index = VariableValue.indexOf(",");
                                ParamValue = ParamValue.replace(UseScriptvariables, VariableValue.substring(index + 1));
                            }
                        }
                        //6.替换环境变量
                        for (Enviromentvariables variables : envvariablesList) {
...
//                        ParamValue=GetDubboValue(ParamValue,ScriptParamsValuesMap, ParamsValuesMap, RadomHashMap, DBParamsValuesMap, GlobalVariablesHashMap, EnvVariablesHashMap, Long.parseLong(enviromentid), projectid);
                        String ParamType = dubboapiCasedata.getParamstype();
                        RequestData = RequestData + "参数名：" + ParamName + "  --------参数值：" + ParamValue + "  -------参数类型：" + ParamType + "\n";
                        ParamsValueList.add(ParamValue);
                        ...
                        if (ParamType.equals("HashMap")) {
                            ParamType = "java.util.HashMap";
                        }
                        if (ParamType.equals("Object")) {
                            ParamType = ParamName;
                        }
                        ParamsTypeList.add(ParamType);
                    }
                    dubboTestResponeData.setRequestData(RequestData);
                    String[] Arry = ParamsTypeList.toArray(new String[ParamsTypeList.size()]);
                    Object[] ObArry = ParamsValueList.toArray();
                    DubboHelp dubboHelp = new DubboHelp();
                    long startTime = System.currentTimeMillis();
                    Object result = null;
                    try {
                        dubboHelp.InitApplication(ModelName);
                        dubboHelp.InitRegister(ConnectStr);
                        dubboHelp.InitIServicegenericService(DeployunitName, ModelName, Version, Group);
                        result = dubboHelp.CallerIServiceMethod(DeployunitName, ModelName, Version, Group, dubboapi.getApiname(), Arry, ObArry);
                    } catch (Exception e) {
                        return ResultGenerator.genFailedResult("Dubbo服务异常：" + e.getMessage());
                    } finally {
                        dubboTestResponeData.setResponeContent(result.toString());
                        long responeTime = System.currentTimeMillis() - startTime;
                        dubboTestResponeData.setResponeTime(responeTime);
                        ScenecasesDebugReport scenecasesDebugReport = new ScenecasesDebugReport();
                        scenecasesDebugReport.setCaseid(Caseid);
                        scenecasesDebugReport.setCasename(Casename);
                        scenecasesDebugReport.setBatchname(batchname);
                        ...
                        scenecasesDebugReport.setLastmodifyTime(new Date());
                        scenecasesDebugReport.setProjectid(projectid);
                        scenecasesDebugReport.setRespone(result.toString());
                        scenecasesDebugReport.setUrl("");
                        scenecasesDebugReport.setRequestdatas(RequestData);
                        scenecasesDebugReport.setRequestmethod("");
                        scenecasesDebugReport.setAssertvalue("");
                        scenecasesDebugReport.setCreator("");
                        scenecasesDebugReport.setErrorinfo("");
                        scenecasesDebugReport.setRequestheader("");
                        scenecasesDebugReport.setExpect("");
                        scenecasesDebugReport.setSceneid(sceneid);
                        scenecasesDebugReport.setScenename(scenename);
                        scenecasesDebugReport.setStatus(Status);
                        scenecasesDebugReport.setRuntime(responeTime);
                        scenecasesDebugReport.setTestplanid(new Long(0));
...
//                        ParamValue=GetDubboValue(ParamValue,ScriptParamsValuesMap, ParamsValuesMap, RadomHashMap, DBParamsValuesMap, GlobalVariablesHashMap, EnvVariablesHashMap, Long.parseLong(enviromentid), projectid);
                    ...
                    long responeTime = System.currentTimeMillis() - startTime;
                    dubboTestResponeData.setResponeTime(responeTime);
                    return ResultGenerator.genOkResult(dubboTestResponeData);
                }
            }
        }
        return ResultGenerator.genOkResult();
    ...
    private String GetDubboValue(String ParamValue, HashMap<String, String> ScriptParamsValuesMap, HashMap<String, String> ParamsValuesMap, HashMap<String, String> RadomMap, HashMap<String, String> DBMap, HashMap<String, String> GlobalVariablesHashMap, HashMap<String, HashMap<Long, String>> EnvVariablesHashMap, Long envid, long projectid) throws Exception {
        ...
        if ((ParamValue.contains("#") && ParamValue.contains("#")) || (ParamValue.contains("{") && ParamValue.contains("}")) || (ParamValue.contains("<") && ParamValue.contains(">")) || (ParamValue.contains("<<") && ParamValue.contains(">>")) || (ParamValue.contains("[") && ParamValue.contains("]")) || (ParamValue.contains("$") && ParamValue.contains("$"))) {
            try {
                Object ob = GetVaraibaleValue(ParamValue, RadomMap, ScriptParamsValuesMap, ParamsValuesMap, DBMap, GlobalVariablesHashMap, EnvVariablesHashMap, envid, projectid);
                Result = ob.toString();
            } catch (Exception ex) {
                Result = ex.getMessage();
            }
