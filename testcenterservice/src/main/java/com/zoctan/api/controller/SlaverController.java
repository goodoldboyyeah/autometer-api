...
public class SlaverController {
    ...
    public Result updateSlaver(@RequestBody final Slaver dic) {
        if(dic.getStype().equals("功能")||dic.getStype().equals("性能"))
        {
            if(!dic.getSlavername().isEmpty())
            {
                this.slaverService.updateSlaver(dic);
                return ResultGenerator.genOkResult();
            }else {
                return ResultGenerator.genFailedResult("请输入执行机名称");
            }
        }else
        {
            return ResultGenerator.genFailedResult("非法类型");
        }
    }
