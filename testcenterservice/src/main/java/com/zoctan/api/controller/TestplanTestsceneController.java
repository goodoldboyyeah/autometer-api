    ...
    public Result addcase(@RequestBody final List<TestplanTestscene> testsceneTestcaseList) {
        if (testsceneTestcaseList.size() > 0) {
            ...
            for (TestplanTestscene tsc : testsceneTestcaseList) {
                ...
                con.createCriteria().andCondition("testplanid = "+planid)
                        .andCondition("testscenenid = " + sceneid);
                if (testplanTestsceneService.listByCondition(con).size() > 0) {
                    return ResultGenerator.genFailedResult("当前集合中已存在此场景："+tsc.getScenename());
                }
                Testscene testscene = testsceneService.getById(sceneid);
            ...
            if (eptype.equals("性能")) {
                if (testsceneTestcaseList.size() > 1) {
                    ...
                    con.createCriteria().andCondition("testplanid = " + executeplan.getId())
                            .andCondition("testscenenid = " + testsceneTestcaseList.get(0).getTestscenenid());
                    List<TestplanTestscene> testplanTestsceneList = testplanTestsceneService.listByCondition(con);
                    if (testplanTestsceneList.size() > 0) {
                        return ResultGenerator.genFailedResult("当前集合中已存在此场景："+testsceneTestcaseList.get(0).getScenename());
                    } else {
