...
public class ExecuteplanController {
    ...
    public Result add(@RequestBody Executeplan executeplan) {
        ...
        if (executeplanService.ifexist(con) > 0) {
            ...
            executeplan.setExecuteplanname(executeplan.getExecuteplanname().replaceAll("\\s+", ""));
    ...
    public Result checkcondition(@RequestBody Executeplan executeplan) throws Exception {
        ...
        for (TestplanTestscene tets : testplanTestsceneList) {
            ...
            for (TestsceneTestcase tt : testsceneTestcaseList) {
                long deployunitid = tt.getDeployunitid();
                Deployunit deployunit = deployunitService.getBy("id", deployunitid);
                if (deployunit == null) {
                    return ResultGenerator.genFailedResult("该测试集合的用例所在的微服务: " + tt.getDeployunitname() + " 未找到，请先添加微服务");
                }
                if(deployunit.getServicetype().equals("Http服务"))
                {
                    ...
                    List<Macdepunit> macdepunitList = macdepunitService.getenvassemblelistbyenvidandtype(envid, "组件");
    ...
    public Result delete(@PathVariable Long id) {
        ...
        if (executeplanbatchList.size() > 0) {
            return ResultGenerator.genFailedResult("当前测试集合有未完成的执行计划，无法删除！！");
        } else {
