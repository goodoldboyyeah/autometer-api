...
import com.zoctan.api.util.SqlserverConnectionUtils;
...
public class TestconditionController {
    ...
    public Result planconditionfinish(@RequestBody Dispatch dispatch) {
        ...
        List<TestconditionReport> testconditionReportList = testconditionReportService.getsubconditionnums(Planid, Batchname, "已完成");
        if (testconditionReportList.size() == SubConditionNums) {
            ...
            if (successtestconditionReportList.size() == SubConditionNums) {
                ...
                TestconditionController.log.info("调度服务【功能】条件报告已成功完成成功的数量: " + successtestconditionReportList.size() + "  条件总条数：" + SubConditionNums);
                return ResultGenerator.genFailedResult(504, "条件报告执行失败");
            }
...
//            requestObject.setSlaverid(Slaverid.toString());
        ...
        if (apicases.getServicetype().equals("Dubbo服务")) {
            ...
            List<Macdepunit> macdepunitList = macdepunitService.getenvassemblelistbyenvidandtype(executeplan.getEnvid(), "组件");
            ...
            if (!existzk) {
                ...
                try {
                    ...
                    ConditionResultStatus = "失败";
                    Respone = "调用dubbo服务端异常："+e.getMessage();
                    TestconditionController.log.info("接口子条件条件DubboHelp异常-============：" + e.getMessage());
                } finally {
                    End = new Date().getTime();
                }
                ...
                if(dubboapi.getResponecontenttype().equals("JSON"))
                {
                    String DubboRespone=JSON.toJSONString(result);
                    Respone = DubboRespone;
                        ...
                        if (testvariables != null) {
                            try {
                                ...
                                switch (VariablesResoruce) {
                                    case "Dubbo":
                                        ParseValue = parseResponeHelp.ParseJsonRespone(VariablesPath, DubboRespone);
                                        break;
                                    default:
                                        ParseValue = parseResponeHelp.ParseJsonRespone(VariablesPath, DubboRespone);
                                }
                                TestvariablesValue testvariablesValue = new TestvariablesValue();
                                testvariablesValue.setPlanid(executeplan.getId());
                                ...
                                testvariablesValue.setPlanname(executeplan.getExecuteplanname());
                                ...
                                VariableNameValueMap.put(testvariablesValue.getVariablesname(), ParseValue);
                            } catch (Exception ex) {
                                Respone = "前置接口条件执行异常，变量:" + testvariables.getTestvariablesname() + " 获取值异常,原因为：" + ex.getMessage();
                            }
                        } else {
                            Respone = "前置接口条件执行异常:接口子条件未找到变量:" + testvariables.getTestvariablesname() + "，请检查变量管理-变量管理中是否存在！";
                        }
                    }
                }
                if(dubboapi.getResponecontenttype().equals("Basic"))
                {
                    ...
                    for (Testvariables testvariables : apicasesVariablesList) {
                        ...
                        testvariablesValue.setVariablesid(testvariables.getId());
                        testvariablesValue.setVariablesname(testvariables.getTestvariablesname());
                        testvariablesValue.setVariablesvalue(result.toString());
                        testvariablesValue.setMemo("test");
                        testvariablesValueService.save(testvariablesValue);
                        VariableNameValueMap.put(testvariablesValue.getVariablesname(), result.toString());
                    }
                }
            }
        }
...
//                }
    ...
    private HashMap<String, HashMap<String, String>> GetCaseApiConditionResult(HashMap<String, HashMap<String, String>> Result, ConditionApi conditionApi, Long Enviromentid) throws Exception {
        ...
        if (apicases.getServicetype().equals("Dubbo服务")) {
            ...
            String ModelName = apicases.getModelname();
            long Dubboapiid = apicases.getApiid();
            List<Macdepunit> macdepunitList = macdepunitService.getenvassemblelistbyenvidandtype(EnviromentID, "组件");
            boolean existzk = false;
            String IP = "";
            String Port = "";
            String ConnectStr = "";
            ...
            if (!existzk) {
                throw new Exception("当前选择环境未配置注册中心,请先完成配置");
            } else {
                Dubboapi dubboapi = dubboapiService.getById(Dubboapiid);
                String Version = dubboapi.getVersion();
                String Group = dubboapi.getDgroup();
                ...
                {
                    throw new Exception("Dubbo服务异常："+e.getMessage());
                }
                //根据用例是否有中间变量，如果有变量，解析（json，xml，html）保存变量值表，没有变量直接保存条件结果表
                Condition con = new Condition(Testvariables.class);
                con.createCriteria().andCondition("caseid = " + apicases.getId());
                ...
                {
                    for (Testvariables testvariables : apicasesVariablesList) {
                        Result.get("api").put(testvariables.getTestvariablesname(), CaseID + "," + result.toString());
                    }
                }
            }
        }
...
//                        long variablesid = dbconditionVariables.getVariablesid();
    ...
    private String Rundb(Dispatch dispatch, ConditionDb conditionDb, String[] ConnetcArray, String AssembleType, String deployunitvisittype, Machine machine, Macdepunit macdepunit, String Sql, String SqlType) throws Exception {
        ...
        if (AssembleType.equalsIgnoreCase("sqlserver")) {
            DBUrl = GetDbUrl(AssembleType, macdepunit, deployunitvisittype, machine, dbname, port);
            SqlserverConnectionUtils.initDbResource(DBUrl, username, pass);
            if (SqlType.equalsIgnoreCase("Select")) {
                //查询语句结果解析到数据库变量中
                // 1.查询数据库条件是否有变量关联
                long Conidtiondbid = conditionDb.getId();
                ...
                if (dbconditionVariablesList.size() > 0) {
                    //2.获取查询结果
                    List<HashMap<String, String>> result = SqlserverConnectionUtils.query(Sql);
                    for (DbconditionVariables dbconditionVariables : dbconditionVariablesList) {
                        long variablesid = dbconditionVariables.getVariablesid();
                        String Variablesname = dbconditionVariables.getVariablesname();
                        String columnname = dbconditionVariables.getFieldname();
                ...
                String[] SqlArr = Sql.split(";");
                for (String ExecSql : SqlArr) {
                    TestconditionController.log.info("数据库子条件sqlserver开始执行：" + ExecSql);
                    int nums = SqlserverConnectionUtils.execsql(ExecSql);
                    TestconditionController.log.info("数据库子条件pgSql执行完成：" + ExecSql);
                    Respone = Respone + " 成功执行Sql:" + Sql + " 影响条数：" + nums;
                }
            }
        ...
        if (AssembleType.equalsIgnoreCase("金仓")) {
            ...
            if (SqlType.equalsIgnoreCase("Select")) {
                ...
                String[] SqlArr = Sql.split(";");
                for (String ExecSql : SqlArr) {
                    TestconditionController.log.info("数据库子条件金仓开始执行：" + ExecSql);
                    int nums = KingbaseConnectionUtils.execsql(ExecSql);
                    TestconditionController.log.info("数据库子条件金仓执行完成：" + ExecSql);
                    Respone = Respone + " 成功执行Sql:" + Sql + " 影响条数：" + nums;
                }
            }
        }
