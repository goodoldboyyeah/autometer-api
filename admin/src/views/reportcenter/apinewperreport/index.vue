        ...
        <el-col :xs="24" :sm="24" :lg="15">
          <div id="12" class="chart-wrapper">
            ...
            >
              ...
              <el-table-column label="线程总数" align="center" prop="threadnums" width="120"/>
              <el-table-column label="循环总数" align="center" prop="loops" width="120"/>
              <el-table-column label="执行机总数" align="center" prop="slavernums" width="120"/>
              <el-table-column label="运行总数" align="center" prop="execCaseNums" width="120"/>
              <el-table-column label="成功总数" align="center" prop="successCaseNums" width="120"/>
              <el-table-column label="失败总数" align="center" prop="failCaseNums" width="120"/>
        ...
        <el-col :xs="24" :sm="24" :lg="15">
          <div id="14" class="chart-wrapper">
            ...
            >
              ...
              <el-table-column label="线程数" align="center" prop="threadnums" width="120"/>
              <el-table-column label="循环数" align="center" prop="loops" width="120"/>
              <el-table-column label="运行总数" align="center" prop="execCaseNums" width="120"/>
              <el-table-column label="成功总数" align="center" prop="successCaseNums" width="120"/>
              <el-table-column label="失败总数" align="center" prop="failCaseNums" width="120"/>
              <el-table-column label="耗时(秒)" align="center" prop="costtime" width="120"/>
            </el-table>
          </div>
        </el-col>
<!--        <el-col :xs="24" :sm="24" :lg="15">-->
...
<!--        </el-col>-->
    ...
    <el-tabs v-model="activeName" type="card" ref="tabs">
      ...
      <el-tab-pane label="Http用例明细报告" name="zero">
        <div class="filter-container">
          <el-form :inline="true">
          <el-form-item label="状态:" prop="testplanname" >
              <el-select v-model="searchhttp.caseststus" style="width:100%" placeholder="值类型">
                <el-option label="成功" value="成功"></el-option>
                <el-option label="失败" value="失败"></el-option>
              </el-select>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="searchhttpcaseReportBy" :loading="btnLoading">查询</el-button>
          </el-form-item>
          </el-form>
        </div>
        <template>
          <el-table
            :data="httpapireportList"
            :key="itemKey"
            v-loading.body="listLoading"
            element-loading-text="loading"
            border
          ...
          >
            <el-table-column label="编号" align="center" width="50">
              <template slot-scope="scope">
                <span v-text="httpgetIndex(scope.$index)"></span>
              </template>
            </el-table-column>
...
<!--            <el-table-column label="测试场景" align="center" prop="scenename" width="80"/>-->
            ...
            <el-table-column label="创建时间" align="center" prop="createTime" width="350">
              <template slot-scope="scope">{{ unix2CurrentTime(scope.row.createTime) }}</template>
            </el-table-column>
          </el-table>
          <el-pagination
            @size-change="httphandleSizeChange"
            @current-change="httphandleCurrentChange"
            :current-page="searchhttp.page"
            :page-size="searchhttp.size"
            :total="httptotal"
            :page-sizes="[10, 20, 30, 40]"
            layout="total, sizes, prev, pager, next, jumper"
          ></el-pagination>
      ...
      <el-tab-pane label="Dubbo用例明细报告" name="three">
        <div class="filter-container">
          <el-form :inline="true">
            <el-form-item label="状态:" prop="testplanname" >
      ...
      <el-tab-pane label="前置条件报告" name="second">
        <template>
          ...
          >
            ...
            <el-table-column label="条件类型" align="center" prop="subconditiontype" width="130"/>
            ...
            <el-table-column label="条件状态" align="center" prop="conditionstatus" width="130">
              <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchhttpcaseReportBy() {
        this.$refs.tmpquery.validate(valid => {
          if (valid) {
            this.tmpquery.servicetype = 'Http服务'
            findApicasereportWithNameandStatus(this.searchhttp).then(response => {
              this.httpapireportList = response.data.list
              this.total = response.data.total
            }).catch(res => {
      ...
      searchcaseReportBy() {
        this.$refs.tmpquery.validate(valid => {
          if (valid) {
            this.tmpquery.servicetype = 'Dubbo服务'
      ...
      testplanselectChanged(e) {
        for (let i = 0; i < this.execplanList.length; i++) {
          if (this.execplanList[i].executeplanname === e) {
            ...
            this.searchhttp.executeplanid = this.execplanList[i].id
      ...
      httpgetIndex(index) {
        return (this.searchhttp.page - 1) * this.searchhttp.size + index + 1
      },
      ...
      handleSizeChange(size) {
        ...
        this.searchcaseReportBy()
      },

      httphandleSizeChange(size) {
        this.searchhttp.page = 1
        this.searchhttp.size = size
        this.searchhttpcaseReportBy()
      },
      ...
      httphandleCurrentChange(page) {
        this.searchhttp.page = page
        this.searchhttpcaseReportBy()
      },
