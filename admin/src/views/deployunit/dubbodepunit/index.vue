      ...
      <el-form :inline="true" style="width: 900px">
        ...
        <el-form-item>
          ...
          <el-button
            ...
            size="mini"
            v-if="hasPermission('depunit:add')"
            @click.native.prevent="showInterfaceDialog"
          >注册中心导入</el-button>
        </el-form-item>
      </el-form>
    </div>
...
<!--      <el-table-column  :show-overflow-tooltip="true"  label="访问端口" align="center" prop="port" width="80"/>-->
      ...
      <el-table-column :show-overflow-tooltip="true"  label="描述" align="center" prop="memo" width="70"/>
...
<!--        </el-form-item>-->
    ...
    <el-dialog title="Interfaces管理" width="800px" :visible.sync="ModelsFormVisible">
      <div class="filter-container">
        <el-form :inline="true">
          <el-form-item>
            <el-button
            ...
            >添加Interfaces</el-button>
          </el-form-item>
          <el-form-item>
            <el-button
              type="primary"
              size="mini"
              @click.native.prevent="showChoseInteraceRegisterDialog"
            >导入Interfaces</el-button>
          </el-form-item>
          <el-form-item>
            <el-button
              type="success"
              size="mini"
              @click.native.prevent="searchdm"
            >刷新</el-button>
          </el-form-item>
        </el-form>
      </div>
      ...
      >
        ...
        <el-table-column :show-overflow-tooltip="true"  label="Interfaces名" align="center" prop="modelname" width="200"/>
    ...
    <el-dialog :title="modeltextMap[modeldialogStatus]" width="800px"  :visible.sync="modeldialogFormVisible">
      <el-form
        ...
        ref="tmpmodel"
      >
        <el-form-item label="Interfaces名" prop="modelname" required>
          <el-input style="width: 500px" placeholder="需要填写完整的接口名+包名"
                    type="text"
                    maxlength="40"
                    prefix-icon="el-icon-edit"
                    auto-complete="off"
        ...
        <el-form-item label="版本" prop="version" required>
          <el-input style="width: 500px" placeholder="版本"
                    type="text"
                    maxlength="40"
                    prefix-icon="el-icon-edit"
                    auto-complete="off"
                    v-model="tmpmodel.version"
          />
        </el-form-item>
        <el-form-item label="分组" prop="dgroup" required>
          <el-input style="width: 500px" placeholder="分组"
                    type="text"
                    maxlength="40"
                    prefix-icon="el-icon-edit"
                    auto-complete="off"
                    v-model="tmpmodel.dgroup"
          />
        </el-form-item>
        <el-form-item label="备注" prop="memo" >
          <el-input style="width: 500px"
    ...
    <el-dialog title="导入" width="800px"  :visible.sync="exportdialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
        label-width="120px"
        style="width: 400px; margin-left:50px;"
        :model="tmpregistercenter"
        ref="tmpregistercenter"
      >
        <el-form-item label="执行环境" prop="enviromentname"  required>
          <el-select v-model="tmpregistercenter.enviromentname" placeholder="执行环境" style="width: 500px" @change="enviromentselectChanged($event)">
        ...
        <el-form-item label="注册中心" prop="assemblename"  required>
          <el-select v-model="tmpregistercenter.assemblename" placeholder="执行环境" style="width: 500px" @change="enviromentassembleselectChanged($event)">
            <div v-for="(assemblename, index) in enviromentassembleList" :key="index">
              <el-option :label="assemblename.assemblename" :value="assemblename.assemblename" required/>
            </div>
          </el-select>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click.native.prevent="exportdialogFormVisible = false">取消</el-button>
        <el-button
          type="success"
          @click.native.prevent="exportfromregister"
        >导入</el-button>
      </div>
    </el-dialog>


    <el-dialog title="注册中心导入" width="800px" :visible.sync="RegisterFormVisible">
      <div class="filter-container">
        <el-form :inline="true">
          <el-form-item>
            <el-button
              ...
              size="mini"
              icon="el-icon-plus"
              @click.native.prevent="showAddModelsRegisterDialog"
            >注册中心导入Interfaces</el-button>
          </el-form-item>
          <el-form-item>
            <el-button
              type="success"
              size="mini"
              @click.native.prevent="getinterfaceList"
            >刷新</el-button>
          </el-form-item>
        </el-form>
      </div>
      ...
      >
        <el-table-column label="编号" align="center" width="45">
          <template slot-scope="scope">
            <span v-text="interfacegetIndex(scope.$index)"></span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true"  label="Interfaces名" align="center" prop="interfacename" width="400"/>
        <el-table-column :show-overflow-tooltip="true"  label="版本" align="center" prop="version" width="160"/>
        <el-table-column :show-overflow-tooltip="true"  label="分组" align="center" prop="dgroup" width="160"/>
      </el-table>
      <el-pagination
        @size-change="searchinterfacehandleSizeChange"
        @current-change="searchinterfacehandleCurrentChange"
        :current-page="searchinterface.page"
        :page-size="searchinterface.size"
        :total="searchinterfacetotal"
        :page-sizes="[10, 20, 30, 40]"
        layout="total, sizes, prev, pager, next, jumper"
      ></el-pagination>
    </el-dialog>

    <el-dialog title="选择Interface" width="800px" :visible.sync="ChoseInterfaceFormVisible">
      <div class="filter-container">
        <el-form :inline="true">
          <el-form-item label="Interface:">
            <el-input clearable v-model="searchinterface.interfacename" clearable @keyup.enter.native="searchInterface" placeholder="Interface"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="searchInterface" :loading="btnLoading">查询</el-button>
          </el-form-item>
        </el-form>
      </div>
      ...
      ></el-pagination>
      <div slot="footer" class="dialog-footer">
        <el-button @click.native.prevent="ChoseInterfaceFormVisible = false">取消</el-button>
        <el-button
          type="success"
          @click.native.prevent="addinterfacemethods"
        >导入</el-button>
      </div>
    </el-dialog>
...
import { search as searchInterface, exportfromregister } from '@/api/deployunit/registercenterinterface'
import { unix2CurrentTime } from '@/utils'
import { mapGetters } from 'vuex'
import { getenviromentallList as getenviromentallList } from '@/api/enviroment/testenviroment'
import { searchdeployunitmodel, adddeployunitmodel, updatedeployunitmodel, removedeployunitmodel, addinterfacemethods } from '@/api/deployunit/depunitmodel'
import { getregister } from '@/api/enviroment/enviromentassemble'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tmpinterfacename: null,
      tmpprotocal: null,
      multipleSelection: [],
      enviromentnameList: [], // 环境列表
      IterfaceList: [],
      RInterfaceList: [],
      enviromentassembleList: [],
      interfacenameList: [],
      depunitList: [], // 服务列表
      modelsList: [], // 服务Interfaces列表
      listLoading: false, // 数据加载等待动画
      total: 0, // 数据总数
      searchinterfacetotal: 0,
      dialogStatus: 'add',
      modeldialogStatus: 'add',
      RegisterFormVisible: false,
      ChoseInterfaceFormVisible: false,
  ...
  created() {
    ...
    this.tmpregistercenter.projectid = window.localStorage.getItem('pid')
    this.searchinterface.projectid = window.localStorage.getItem('pid')
    // this.getdepunitList()
  ...
  methods: {
    ...
    addinterfacemethods() {
      ...
      if (this.multipleSelection.length === 0) {
        this.$message.error('请选择导入的Interface')
      } else {
        for (let i = 0; i < this.multipleSelection.length; i++) {
          this.RInterfaceList.push({
            'interfacename': this.multipleSelection[i].interfacename,
            ...
            'servicetype': 'Dubbo服务',
            'deployunitid': this.tmpdeploymodel.deployunitid
          })
        }
        addinterfacemethods(this.RInterfaceList).then(() => {
          this.ChoseInterfaceFormVisible = false
    ...
    enviromentselectChanged(e) {
      for (let i = 0; i < this.enviromentnameList.length; i++) {
        if (this.enviromentnameList[i].enviromentname === e) {
          this.tmpregistercenter.envid = this.enviromentnameList[i].id
        }
        // 通过环境id获取注册中心
        this.getregister(this.tmpregistercenter)
        console.log(this.enviromentnameList[i].id)
      }
    },

    enviromentassembleselectChanged(e) {
      for (let i = 0; i < this.enviromentassembleList.length; i++) {
        if (this.enviromentassembleList[i].assemblename === e) {
          this.tmpregistercenter.assembleid = this.enviromentassembleList[i].id
        }
      }
    },
    ...
    getregister() {
      getregister(this.tmpregistercenter).then(response => {
        this.enviromentassembleList = response.data
      }).catch(res => {
        this.$message.error('加载环境列表失败')
      })
    },
    ...
    getinterfaceList() {
      this.searchinterface.interfacename = this.tmpinterfacename
      searchInterface(this.searchinterface).then(response => {
        this.interfacenameList = response.data.list
        this.searchinterfacetotal = response.data.total
      }).catch(res => {
        this.$message.error('加载服务列表失败')
      })
    },
    ...
    searchInterface() {
      this.search.page = 1
      searchInterface(this.search).then(response => {
        this.interfacenameList = response.data.list
        this.searchinterfacetotal = response.data.total
      }).catch(res => {
        this.$message.error('搜索失败')
      })
      this.tmpinterfacename = this.searchinterface.interfacename
    },
    ...
    searchdeployunitmodel() {
      searchdeployunitmodel(this.tmpdeploymodel).then(response => {
        ...
        this.$message.error('加载服务Interfaces列表失败')
      })
    },
    ...
    exportfromregister() {
      exportfromregister(this.tmpregistercenter)
      this.exportdialogFormVisible = false
      this.$message.success('导入中。。。。。请稍后刷新页面查看')
      // exportfromregister(this.tmpregistercenter).then(response => {
      //   this.IterfaceList = response.data.list
      //   this.total = response.data.total
      //   this.exportdialogFormVisible = false
      // }).catch(res => {
      //   this.$message.error('搜索失败')
    ...
    searchinterfacehandleSizeChange(size) {
      this.searchinterface.page = 1
      this.searchinterface.size = size
      this.getinterfaceList()
    },

    searchinterfacehandleCurrentChange(page) {
      this.searchinterface.page = page
      this.getinterfaceList()
    },
    /**
     * 表格序号
    ...
    interfacegetIndex(index) {
      return (this.searchinterface.page - 1) * this.searchinterface.size + index + 1
    },
    /**
     * 显示添加服务对话框
     */
    ...
    showUpdatedepunitmodelDialog(index) {
      ...
      this.tmpmodel.version = this.modelsList[index].version
      this.tmpmodel.dgroup = this.modelsList[index].dgroup
    ...
    showmodelsDialog(index) {
      ...
      this.tmpregistercenter.deployunitid = this.depunitList[index].id
      this.searchdeployunitmodel()
    },
    ...
    showAddModelsDialog(index) {
      ...
      this.tmpmodel.version = ''
      this.tmpmodel.dgroup = ''
      this.tmpmodel.memo = ''
      this.tmpmodel.creator = this.name
    },

    showAddModelsRegisterDialog(index) {
      // 显示新增对话框
      this.exportdialogFormVisible = true
      this.tmpregistercenter.enviromentname = ''
      this.tmpregistercenter.assemblename = ''
      this.tmpregistercenter.envid = ''
      this.tmpregistercenter.assembleid = ''
    },

    showChoseInteraceRegisterDialog(index) {
      // 显示新增对话框
      this.ChoseInterfaceFormVisible = true
