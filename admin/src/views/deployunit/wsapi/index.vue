  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" style="width: 1200px">
        <span v-if="hasPermission('wsapi:search')">
    ...
    >
      ...
      <el-table-column label="Path" align="center" prop="path" width="80">
        <template slot-scope="scope">
...
<!--      <el-table-column :show-overflow-tooltip="true"   label="测试维护人" align="center" prop="mnickname" width="90"/>-->
      ...
      <el-table-column label="管理" align="center"  :min-width="dynamicWidth"
                       v-if="hasPermission('wsapi:update')  || hasPermission('wsapi:delete')">
        <template slot-scope="scope">
          ...
          <el-button
            type="warning"
            size="mini"
            v-if="hasPermission('wsapi:update') && scope.row.id !== id"
          ...
          <el-button
            type="primary"
            size="mini"
            v-if="hasPermission('wsapi:delete') && scope.row.id !== id"
    ...
    <el-dialog :title="textMap[dialogStatus]"  width="800px"  :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="请求格式:" prop="requestcontenttype" required>
          <el-select v-model="tmpapi.requestcontenttype" style="width: 500px" placeholder="api风格">
            <el-option label="文本" value="文本"></el-option>
            <el-option label="二进制" value="二进制"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="响应格式:" prop="responecontenttype" required>
          <el-select v-model="tmpapi.responecontenttype" style="width: 500px" placeholder="api风格">
            <el-option label="文本" value="文本"></el-option>
            <el-option label="二进制" value="二进制"></el-option>
          </el-select>
        </el-form-item>
                ...
                <template>
                  <div>
                    <el-form
                      :model="tmpapiparams"
                      ref="tmpapiparams">
                      <el-form-item  prop="keydefaultvalue">
...
<!--                        </el-button>-->
                        <el-input
                          ...
                          prefix-icon="el-icon-message"
                          auto-complete="off"
                          v-model="tmpapiparams.keydefaultvalue"
                        />
                      </el-form-item>
                    </el-form>
                  </div>
...
import { search, addapi, updateapi, removeapi, getapisbydeployunitid, copyapi, removebatchapi } from '@/api/deployunit/wsapi'
...
} from '@/api/deployunit/wsapiparams'
...
export default {
  ...
  methods: {
     ...
     */
    searchparamsbyapiid() {
      searchparamsbyapiid(this.tmpapiparams).then(response => {
        if (response.data.list.length !== 0) {
          this.tmpapiparams = response.data.list[0]
        }
      }).catch(res => {
    ...
    ShowNewParamsDialog(index) {
      ...
      this.tmpapiparams.apiid = this.apiList[index].id
      this.tmpapiparams.apiname = this.apiList[index].apiname
      this.tmpapiparams.deployunitid = this.apiList[index].deployunitid
      this.tmpapiparams.deployunitname = this.apiList[index].deployunitname
    ...
    showUpdateapiDialog(index) {
      ...
      this.tmpapi.projectid = window.localStorage.getItem('pid')
      this.tmpmodelquery.deployunitid = this.tmpapi.deployunitid
      searchdeployunitmodel(this.tmpmodelquery).then(response => {
        this.modelList = response.data.list
      }).catch(res => {
        this.$message.error('加载服务模块列表失败')
