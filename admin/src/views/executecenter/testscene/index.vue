...
<!--        </el-form-item>-->
    ...
    <el-dialog width="1100px" title='场景用例' :visible.sync="testscenecasedialogFormVisible">
      <div class="filter-container" >
        <el-form :inline="true"  >
          <el-form-item  label="服务类型:" prop="servicetype" >
            <el-select style="width: 120px" v-model="searchcase.servicetype" filterable clearable  placeholder="服务类型" @change="servicetypeselectChanged($event)">
...
<!--          </el-form-item>-->
    ...
    <el-dialog width="900px" title='添加用例' :visible.sync="addtestscenecasedialogFormVisible">
      <div class="filter-container" >
        <el-form :inline="true"  >
          <el-form-item  label="服务类型:" prop="servicetype" >
            <el-select style="width: 120px" v-model="addsearchcase.servicetype" filterable clearable  placeholder="服务类型" @change="servicetypeselectChanged($event)">
              <div v-for="(servicetype, index) in servicetypeList" :key="index">
                <el-option :label="servicetype.dicitmevalue" :value="servicetype.dicitmevalue" />
              </div>
            </el-select>
...
<!--            >使用变量</el-button>-->
    ...
    <el-dialog title="Dubbo用例值" width="1000px" :visible.sync="casedataialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
    ...
    <el-dialog title="Http用例值" width="1000px" :visible.sync="HttpcasedataialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
        ...
        <template>
          <el-tabs v-model="opearationactiveName" type="card" ref="tabs">
            <el-tab-pane label="用例值" name="zero">
              ...
              <div slot="footer" class="dialog-footer">
                <el-button @click.native.prevent="HttpcasedataialogFormVisible = false">取消</el-button>
                <el-button
                  type="success"
                  :loading="btnLoading"
                  @click.native.prevent="addapicasesdata"
            ...
            <el-tabs v-model="activebbbName" type="card" >
              <el-tab-pane label="用例数据" name="xxx">
                <el-tabs v-model="activeaaaName" type="card" ref="tabs">
                  <el-tab-pane label="Header" v-if="showhttp" name="zero">
                    <template>
                      <el-table :data="Headertabledatas" fit
                                highlight-current-row border>
                        <el-table-column label="参数" prop="apiparam" align="center" width="300">
                  ...
                  <el-tab-pane label="Params" v-if="showhttp" name="first">
                    <template>
                      <el-table :data="Paramstabledatas" border>
                        <el-table-column label="参数"  align="center" width="200">
                          <template slot-scope="scope">
                  ...
                  <el-tab-pane label="Body" v-if="showhttp" name="second">
                    <template>
                      <div v-if="BodyParamDataVisible">
                        <el-table :data="Bodytabledatas" border>
                          <el-table-column label="参数"  align="center">
                  ...
                  <el-tab-pane label="参数值" v-if="showdubbo" name="zero">
                    <template>
                      <el-table :data="dubbocasedatasList" border>
                        <el-table-column label="参数"  align="center">
                          <template slot-scope="scope">
      ...
      <div style="height: 350px; margin-top: 20px">
        <el-form :inline="true">
          ...
          >
            ...
            <el-table-column :show-overflow-tooltip="true"  label="测试场景" align="center" prop="scenename" width="120"/>
            <el-table-column :show-overflow-tooltip="true"  label="用例名" align="center" prop="casename" width="100"/>
...
export default {
  ...
  data() {
    return {
      ...
      HttpcasedataialogFormVisible: false,
  ...
  created() {
    ...
    this.tmpcopyscene.creatorid = this.accountId
  ...
  methods: {
    ...
    getcasedatabycaseid() {
      console.log(this.tmpapicases)
      var casedata = { caseid: this.tmpapicase.caseid }
    ...
    showApi(e) {
      ...
      if (e.servicetype === 'Dubbo服务') {
        this.$router.push({ path: '/deployunitapi/      multipleSelection: [],\n/list', query: { apiname: e.apiname }})
      }
    },
    showCase(e) {
      ...
      if (e.servicetype === 'Dubbo服务') {
        this.$router.push({ path: '/executecenter/dubboapicases/list', query: { casename: e.casename }})
      }
    },
    showDeployUnit(e) {
    ...
    async clickNode(data, node, obj) {
      ...
      if (node.level === 1) {
        ...
        this.tmpapicase.servicetype = node.data.servicetype
        this.tmpapicase.apiid = node.data.apiid
        if (this.tmpapicase.servicetype === 'Http服务') {
          this.showhttp = true
          this.showdubbo = false
        ...
        if (this.tmpapicase.servicetype === 'Dubbo服务') {
          this.showhttp = false
          this.showdubbo = true
        ...
        this.tmpassert.caseid = node.data.caseid
        this.tmpdbassert.caseid = node.data.caseid
        this.searchassert.caseid = node.data.caseid
        this.searchdbassert.caseid = node.data.caseid
        this.getassertbycaseid(this.searchassert)
        this.getdbassertbycaseid(this.searchdbassert)
    ...
    showCaseVariablesDialog(index) {
      this.caseVariablesDialogFormVisible = true
      this.tmptestvariables.caseid = this.testscenecaseList[index].testcaseid
      this.tmptestvariables.casename = this.testscenecaseList[index].casename
      this.findtestvariablesbycaseid()
    },
    ...
    async addapicasesdata() {
      ...
      this.HttpcasedataialogFormVisible = false
    },
    ...
    async showtestcasedataDialog(index) {
      ...
      this.tmpapicase.servicetype = this.testscenecaseList[index].servicetype
      this.tmpapicase.apiid = this.testscenecaseList[index].apiid
      if (this.testscenecaseList[index].servicetype === 'Http服务') {
        this.HttpcasedataialogFormVisible = true
      ...
      if (this.testscenecaseList[index].servicetype === 'Dubbo服务') {
      ...
      if (this.casemultipleSelection.length === 0) {
        ...
        for (let i = 0; i < this.casemultipleSelection.length; i++) {
          this.testcaseList.push({
            ...
            'servicetype': this.casemultipleSelection[i].servicetype,
    ...
    addcasedeployunitselectChanged(e) {
      ...
      if (this.addsearchcase.deployunitid !== 0) {
        ...
        if (this.addsearchcase.servicetype === 'Http服务') {
        ...
        if (this.addsearchcase.servicetype === 'Dubbo服务') {
          getdubboapiListbydeploy(this.addsearchcase).then(response => {
            this.addcaseapiList = response.data
          }).catch(res => {
            this.$message.error('加载api列表失败')
          })
