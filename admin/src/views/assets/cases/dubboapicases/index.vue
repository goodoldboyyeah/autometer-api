...
<!--      <el-form :inline="true">-->
        <span v-if="hasPermission('apicases:search')" >
          ...
          <el-form-item label="Interface：">
            <el-select style="width: 120px" v-model="search.modelname" filterable placeholder="Interface" clearable @change="searchmodelselectChanged($event)">
          ...
          <el-select style="width: 120px" clearable v-model="search.protocal" placeholder="协议" >
...
<!--      <el-table-column :show-overflow-tooltip="true" label="服务类型" align="center" prop="servicetype" width="85">-->
<!--      </el-table-column>-->
      <el-table-column :show-overflow-tooltip="true" label="协议" align="center" prop="protocal" width="80"/>
      <el-table-column :show-overflow-tooltip="true" label="Interface" align="center" prop="modelname" width="100"/>
...
<!--      </el-table-column>-->
    ...
    <el-dialog :title="textMap[dialogStatus]"  width="800px"  :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="Interface" prop="modelname" required >
          <el-select v-model="tmpapicases.modelname" clearable  style="width: 500px" placeholder="Interface" @change="modelselectChanged($event)">
            <el-option label="请选择" value="''" style="display: none" />
            <div v-for="(model, index) in modelList" :key="index">
...
<!--            >添加前置延时</el-button>-->
    ...
    <el-dialog :title="apiconditiontextMap[apiconditiondialogStatus]" width="800px"  :visible.sync="caseconditiondialogFormVisible">
      ...
      >
        ...
        <el-form-item  label="服务类型:" prop="servicetype" >
          <el-select  v-model="tmpapicondition.servicetype" filterable clearable  placeholder="服务类型"  style="width: 500px"  @change="servicetypeselectChanged($event)">
            <div v-for="(servicetype, index) in servicetypeList" :key="index">
              <el-option :label="servicetype.dicitmevalue" :value="servicetype.dicitmevalue" />
            </div>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="Http用例操作"  width="1000px"  :visible.sync="caseoperationialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
    ...
    <el-dialog title="Dubbo用例操作"  width="1000px"  :visible.sync="dubbocaseoperationialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
          ...
          <el-tabs v-model="opearationactiveName" type="card" ref="tabs">
            <el-tab-pane label="用例值" name="zero">
              ...
              >
                <template>
                  <el-tabs v-model="activeName" type="card" ref="tabs">
                    <el-tab-pane label="参数值" name="zero">
                      <template>
                        <el-table :data="dubbocasedatasList" border>
                          <el-table-column label="参数"  align="center">
                            <template slot-scope="scope">
                              <el-input size="mini" readonly="true" v-model="scope.row.apiparam"></el-input>
                            </template>
                ...
                >
                  <div class="filter-container">
                    <el-form :inline="true" :model="tmptest" ref="tmptest">
                      ...
                      <el-form-item label="环境：" prop="enviromentname"  required>
                        <el-select style="width:650px" v-model="tmptest.enviromentname"  placeholder="环境" @change="EnviromentselectChanged($event)" >
                          <el-option label="请选择"  value=""  />
                          <div v-for="(enviroment, index) in enviromentnameList" :key="index">
                            <el-option :label="enviroment.enviromentname" :value="enviroment.enviromentname" required />
                          </div>
                        </el-select>
                        <el-button type="primary" :loading="btnLoading" @click.native.prevent="rundubbotest">调试</el-button>
                        <el-button type="primary" :loading="btnLoading" @click.native.prevent="showtestcaseConditionDialog">前置条件</el-button>
                      </el-form-item>
                        ...
                        <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                          <el-tab-pane label="通用" name="zero">
                            <el-input
                              ...
                              rows="20" cols="90"
                              maxlength="4000"
                              v-model="tmpdubbotest.general"
                            />
                          </el-tab-pane>
                          <el-tab-pane label="请求参数" name="seven">
                            <el-input
                              type="textarea"
                              style="width: 100%;border: none;outline: none;resize:none;overflow:hidden" readonly
                              rows="20" cols="90"
                              maxlength="4000"
                              v-model="tmpdubbotest.RequestData"
                            />
                          </el-tab-pane>
                          <el-tab-pane label="响应内容" name="first">
                            <el-input
                              ...
                              rows="20" cols="90"
                              maxlength="4000"
                              v-model="tmpdubbotest.ResponeContent"
                            />
                          </el-tab-pane>
                          <el-tab-pane label="响应时间(ms)" name="third">
                            <el-input
                              ...
                              rows="20" cols="90"
                              maxlength="4000"
                              v-model="tmpdubbotest.ResponeTime"
                            />
                          </el-tab-pane>
                        </el-tabs>
                      </template>
                  ...
                  <el-tab-pane label="接口断言" name="zero">
                    <template>
                        ...
                        <el-form :inline="true"  :model="searchassert" ref="searchcase" >
                          <el-form-item>
                            <el-button
                              ...
                              size="mini"
                              icon="el-icon-plus"
                              @click.native.prevent="showAddDubboassertDialog"
                            >添加接口断言
                            </el-button>
                          </el-form-item>
                          ...
                          <el-form-item label="接口断言类型:"  prop="asserttype" >
                            <el-select v-model="searchassert.asserttype" placeholder="断言类型" >
                              <el-option label="基础类型断言" value="Basic"></el-option>
                              <el-option label="对象断言" value="Json"></el-option>
                            </el-select>
                          </el-form-item>
                      ...
                      >
                        ...
                        <el-table-column label="断言类型"  align="center" prop="asserttype" width="80"/>
                        <el-table-column :show-overflow-tooltip="true"  label="表达式"  align="center" prop="expression" width="200"/>
                        <el-table-column label="条件" align="center" prop="assertcondition" width="70"/>
                        <el-table-column :show-overflow-tooltip="true"  label="期望值"  align="center" prop="assertvalues" width="200"/>
                        <el-table-column label="断言值类型"  align="center" prop="assertvaluetype" width="100"/>
                        <el-table-column label="管理" align="center"  width="180">
                          <template slot-scope="scope">
                            <el-button
                              type="warning"
                              size="mini"
                              @click.native.prevent="showUpdateDubboapicasesassertDialog(scope.$index)"
                            >修改
                            </el-button>
                            <el-button
                              type="danger"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          @click.native.prevent="dubbocaseoperationialogFormVisible = false">取消</el-button>
        <el-button
          type="success"
          :loading="btnLoading"
          @click.native.prevent="addnewapicasesdata"
    ...
    <el-dialog :title="DubboAsserttextMap[DubboAssertdialogStatus]" width="800px"  :visible.sync="DubboAssertAUdialogFormVisible">
      <el-form
        status-icon
        class="small-space"
        label-position="left"
      ...
      >
        <el-form-item label="断言类型" prop="asserttype" required >
          <el-select v-model="tmpassert.asserttype"  style="width: 500px" placeholder="断言类型" @change="DubboasserttypeselectChanged($event)">
            <el-option label="基础类型断言" value="Basic"></el-option>
            <el-option label="对象断言" value="Json"></el-option>
          </el-select>
        </el-form-item>
        ...
        <div v-if="DubboExpressionVisible">
          <el-form-item label="表达式" prop="expression" required>
            <el-input style="width: 500px"
                      type="text"
                      maxlength="400"
            ...
            <div class="right">
              <el-tooltip placement="right-end">
                <div slot="content">1.如果断言类型是Json则使用JsonPath表示, 例如：$.store.book[0].title  在线解析网站：http://www.e123456.com/aaaphp/online/jsonpath/<br/></div>
                <el-button>表达式语法</el-button>
              </el-tooltip>
            </div>
          </el-form-item>
        ...
        <el-form-item label="期望值" prop="assertvalues" required>
          <el-input style="width: 500px"
                    type="textarea"
                    maxlength="2000"
                    prefix-icon="el-icon-edit"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click.native.prevent="DubboAssertAUdialogFormVisible = false">取消</el-button>
        <el-button
          type="success"
          :loading="btnLoading"
          v-if="DubboAssertdialogStatus === 'add'"
          @click.native.prevent="addassert"
        >保存
        </el-button>
        <el-button
          type="success"
          :loading="btnLoading"
          v-if="DubboAssertdialogStatus === 'update'"
          @click.native.prevent="updateassert"
        >更新
        </el-button>
      </div>
...
<script>
  ...
  import { getcasedatabycaseid, updatedubbocasedata } from '@/api/assets/dubboapicasesdata'
  import { getapiListbydeploy as getapiListbydeploy, getapi } from '@/api/deployunit/api'
  import { getdubboapiListbydeploy as getdubboapiListbydeploy } from '@/api/deployunit/dubboapi'
  ...
  export default {
    ...
    data() {
      return {
        ...
        dubbocaseoperationialogFormVisible: false,
        accountList: [],
        tmpdbassertvalue: {
          id: '',
          dbassertid: '',
        ...
        DubboAssertAUdialogFormVisible: false,
        DubboAsserttextMap: {
          update: '修改Dubbo断言值',
          add: '添加Dubbo断言值'
        },
    ...
    methods: {
      servicetypeselectChanged(e) {
        this.deployunitList = null
        this.search.deployunitname = null
        this.tmpapicases.deployunitname = null
        this.search.servicetype = e
        this.getdepunitLists()
      },
      getservicetypeQueryList() {
        getdatabydiccodeList(this.servicetypeQuery).then(response => {
          this.servicetypeList = response.data.list
        }).catch(res => {
      ...
      showApi(e) {
        ...
        this.$router.push({ path: '/deployunitapi/dubboapi/list', query: { apiname: e.apiname }})
      },
      showDeployUnit(e) {
      ...
      showAddDubboassertDialog() {
        // 显示新增对话框
        this.DubboAssertdialogStatus = 'add'
        this.DubboAssertAUdialogFormVisible = true
        this.tmpassert.id = ''
        this.tmpassert.assertcondition = ''
        this.tmpassert.assertsubtype = ''
        this.tmpassert.asserttype = ''
      ...
      apiconditionmodelselectChanged(e) {
        ...
        if (this.tmpapicondition.servicetype === 'Dubbo服务') {
          getdubboapiListbydeploy(this.tmpapicondition).then(response => {
            this.apiconditionapiList = response.data
          }).catch(res => {
            this.$message.error('加载Dubboapi列表失败')
          })
        }
        if (this.tmpapicondition.servicetype === 'Http服务') {
          getapiListbydeploy(this.tmpapicondition).then(response => {
            this.apiconditionapiList = response.data
          }).catch(res => {
            this.$message.error('加载Httpapi列表失败')
          })
        }
        // getapiListbydeploy(this.tmpapicondition).then(response => {
        //   this.apiconditionapiList = response.data
      ...
      showUpdateapiconditionDialog(index) {
        ...
        this.tmpapicondition.servicetype = this.apiconditioncaseList[index].servicetype
      ...
      ShowAddcasecaseconditionDialog(index) {
        ...
        this.tmpapicondition.modelname = ''
        this.tmpapicondition.servicetype = ''
      ...
      getcasedatabycaseid() {
        var casedata = { caseid: this.tmpapicases.id }
        getcasedatabycaseid(casedata).then(response => {
          this.dubbocasedatasList = response.data.list
        }).catch(res => {
          this.$message.error('获取Dubbo参数值失败')
        })
      },
      /**
       * 获取Params参数值
      ...
      DubboasserttypeselectChanged(e) {
        if (e === 'Basic') {
          this.DubboExpressionVisible = false
        } else {
          this.DubboExpressionVisible = true
        }
        this.tmpassert.assertsubtype = ''
        this.tmpassert.assertcondition = ''
        this.tmpassert.expression = ''
      ...
      getapicasesList() {
        ...
        this.search.protocal = this.tmpsearchprotocal
        this.search.path = this.tmppath
        this.search.servicetype = 'Dubbo服务'
      ...
      selectChanged(e) {
        ...
        for (let i = 0; i < this.deployunitList.length; i++) {
          if (this.deployunitList[i].deployunitname === e) {
            ...
            console.log(this.tmpmodelquery.deployunitid)
          }
        }
      ...
      modelselectChanged(e) {
        ...
        if (this.tmpapicases.servicetype === 'Http服务') {
          getapiListbydeploy(this.apiQuery).then(response => {
            this.apiList = response.data
          }).catch(res => {
            this.$message.error('加载api列表失败')
        ...
        if (this.tmpapicases.servicetype === 'Dubbo服务') {
          getdubboapiListbydeploy(this.apiQuery).then(response => {
            this.apiList = response.data
          }).catch(res => {
            this.$message.error('加载api列表失败')
          })
      ...
      searchmodelselectChanged(e) {
        ...
        getdubboapiListbydeploy(this.search).then(response => {
          this.apiList = response.data
        }).catch(res => {
          this.$message.error('加载api列表失败')
        })
      ...
      searchBy() {
        ...
        this.tmpsearchprotocal = this.search.protocal
        this.tmpservicetype = 'Dubbo服务'
      },
        ...
        this.$refs.tmpassert.validate(valid => {
          if (valid) {
            addapicasesassert(this.tmpassert).then(() => {
              ...
              this.DubboAssertAUdialogFormVisible = false
            }).catch(res => {
      ...
      rundubbotest() {
        this.$refs.tmptest.validate(valid => {
          if (valid) {
            runtest(this.tmptestdata).then(response => {
              this.tmpdubbotest.general = response.data.general
              this.tmpdubbotest.RequestData = response.data.requestData
              this.tmpdubbotest.ResponeContent = response.data.responeContent
              this.tmpdubbotest.ResponeTime = response.data.responeTime
            }).catch(res => {
              this.$message.error('调试失败')
            })
          }
      ...
      async addnewapicasesdata() {
        if (this.tmpapicases.servicetype === 'Http服务') {
          ...
          this.caseoperationialogFormVisible = false
        }
        if (this.tmpapicases.servicetype === 'Dubbo服务') {
          this.updatedubbocasedata(this.dubbocasedatasList)
          this.dubbocaseoperationialogFormVisible = false
        }
      ...
      updatedubbocasedata(datas) {
        updatedubbocasedata(datas).then(response => {
        }).catch(res => {
          this.$message.error('更新Dubbo用例值失败')
        })
      },
      /**
       * 显示修改用例对话框
      ...
      showUpdateapicasesDialog(index) {
        ...
        this.tmpapicases.servicetype = this.apicasesList[index].servicetype
        ...
        this.apiQuery.modelid = 0
        this.search.servicetype = this.tmpapicases.servicetype
        this.getdepunitLists()
        // for (let i = 0; i < this.deployunitList.length; i++) {
        ...
        //   }
        // }
        this.apiQuery.deployunitid = this.tmpapicases.deployunitid
        this.tmpmodelquery.deployunitid = this.tmpapicases.deployunitid
      ...
      async showcaseoperationDialog(index) {
        ...
        this.tmpapicases.apiid = this.apicasesList[index].apiid
        this.tmpapicases.servicetype = this.apicasesList[index].servicetype
        if (this.apicasesList[index].servicetype === 'Http服务') {
          ...
          this.caseoperationialogFormVisible = false
          this.dubbocaseoperationialogFormVisible = true
          this.getcasedatabycaseid()
        }
        // 调试
        ...
        this.tmptest.respone = ''
        this.tmpdubbotest.RequestData = ''
        this.tmpdubbotest.ResponeContent = ''
        this.tmpdubbotest.ResponeTime = ''
        this.tmpdubbotest.general = ''
        this.tmptest.responeTime = ''
        this.tmptest.enviromentname = ''
        this.tmptest.respone = ''
      ...
      showUpdateDubboapicasesassertDialog(index) {
        this.DubboAssertdialogStatus = 'update'
        this.DubboAssertAUdialogFormVisible = true
        this.tmpassert.id = this.assertList[index].id
        this.tmpassert.asserttype = this.assertList[index].asserttype
        this.tmpassert.assertsubtype = this.assertList[index].assertsubtype
        ...
        this.tmpassert.assertvaluetype = this.assertList[index].assertvaluetype
        this.tmpassert.creator = this.name
        if (this.tmpassert.asserttype === 'Basic') {
          this.DubboExpressionVisible = false
        } else {
          this.DubboExpressionVisible = true
        }
